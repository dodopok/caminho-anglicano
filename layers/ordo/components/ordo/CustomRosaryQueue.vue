...
<template>
  <section class="ordo-queue-card">
    <div class="ordo-queue-card__header"><div><p class="ordo-kicker">Revisão editorial</p><h2>Rosários compartilhados</h2><span>Detalhe expandido pronto para transcrição no Strapi</span></div><span class="ordo-queue-card__count">{{ formatNumber(summary?.created_in_period) }}</span></div>
    ...
    <div v-else-if="rosaries.length" class="ordo-queue-list"><button v-for="rosary in rosaries.slice(0, 5)" :key="rosary.id" type="button" class="ordo-queue-item ordo-queue-item--button" @click="emit('open', rosary)"><div class="ordo-queue-item__mark ordo-queue-item__mark--warm">✦</div><div class="ordo-queue-item__copy"><div><strong>{{ rosary.title }}</strong><span :class="['ordo-status', statusClass(rosary.share_status)]">{{ statusLabel(rosary.share_status) }}</span></div><p>{{ rosary.description || 'Sem descrição' }}</p><small>{{ rosary.author?.name || 'Autor não informado' }} · {{ rosary.locale || 'locale não informado' }} · {{ formatDate(rosary.created_at) }}</small></div><span class="ordo-queue-item__arrow">↗</span></button></div>
    <div v-else class="ordo-empty-inline"><span class="ordo-empty-inline__mark">∅</span><span>Nenhuma oração nesta fila.</span></div>
