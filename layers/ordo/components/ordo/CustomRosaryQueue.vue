...
const pageSize = 5
const queueTotal = computed(() => props.rosaries.length)
const visibleRosaries = computed(() => props.rosaries.slice((props.currentPage - 1) * pageSize, props.currentPage * pageSize))
...
<template>
  <section class="ordo-queue-card">
    <div class="ordo-queue-card__header"><div><p class="ordo-kicker">Revisão editorial</p><h2>Rosários compartilhados</h2><span>{{ formatNumber(visibleRosaries.length) }} nesta página · {{ formatNumber(queueTotal) }} no total</span></div><div class="ordo-table-card__header-actions"><button v-if="!loading && hasHiddenRosaries" type="button" class="ordo-card-action" @click="emit('open-all')">Abrir tabela ↗</button><span class="ordo-queue-card__count">{{ formatNumber(visibleRosaries.length) }}</span></div></div>
